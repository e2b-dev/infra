...
@test "main refuses a non-x86_64 host with FIX" {
  run env -u FA_NO_MAIN FA_ARCH=aarch64 HOST_ROOT="$HOST_ROOT" bash "$BATS_TEST_DIRNAME/../compose/scripts/fetch-artifacts.sh"
  [ "$status" -eq 1 ]
  [[ "$output" == *"FIX: only x86_64 hosts are supported"* ]]
}
...
@test "main fails with the exact FIX line for an unrecognized architecture" {
  ...
  [ "$status" -eq 1 ]
  [[ "$output" == *"FIX: only x86_64 hosts are supported; the released orchestrator and envd have no mips build"* ]]
}
