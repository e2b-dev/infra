...
func TestVolumeRoundTrip(t *testing.T) {
	...
	// create volume
	createVolume, err := client.PostVolumesWithResponse(t.Context(), api.PostVolumesJSONRequestBody{
		Name: volumeName,
	})
	...
	createVolumeFailure, err := client.PostVolumesWithResponse(t.Context(), api.PostVolumesJSONRequestBody{
		Name: volumeName,
	})
	require.NoError(t, err)
	...
	// retrieve volume
	getVolume, err := client.GetVolumesVolumeIDWithResponse(t.Context(), volumeID)
	require.NoError(t, err)
	...
	// list volumes
	listVolumes, err := client.GetVolumesWithResponse(t.Context())
	require.NoError(t, err)
