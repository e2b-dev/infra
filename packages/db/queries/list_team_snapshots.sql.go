...
// versions:
//   sqlc v1.29.0
// source: list_team_snapshots.sql
...
const listTeamSnapshots = `-- name: ListTeamSnapshots :many
SELECT 
    e.id as snapshot_id,
    st.sandbox_id,
    e.created_at,
...
type ListTeamSnapshotsParams struct {
	TeamID     uuid.UUID
	SandboxID  *string
...
type ListTeamSnapshotsRow struct {
	SnapshotID         string
	SandboxID          string
	CreatedAt          time.Time
	TeamID             uuid.UUID
...
func (q *Queries) ListTeamSnapshots(ctx context.Context, arg ListTeamSnapshotsParams) ([]ListTeamSnapshotsRow, error) {
	rows, err := q.db.Query(ctx, listTeamSnapshots,
		arg.TeamID,
		arg.SandboxID,
		arg.CursorTime,
		arg.CursorID,
	...
	defer rows.Close()
	var items []ListTeamSnapshotsRow
	for rows.Next() {
		var i ListTeamSnapshotsRow
		if err := rows.Scan(
			&i.SnapshotID,
			&i.SandboxID,
			&i.CreatedAt,
