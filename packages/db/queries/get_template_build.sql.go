...
const getTemplateBuildWithEnv = `-- name: GetTemplateBuildWithEnv :one
...
type GetTemplateBuildWithEnvParams struct {
	TemplateID string
	BuildID    uuid.UUID
}

type GetTemplateBuildWithEnvRow struct {
	Env      Env
	EnvBuild EnvBuild
}

func (q *Queries) GetTemplateBuildWithEnv(ctx context.Context, arg GetTemplateBuildWithEnvParams) (GetTemplateBuildWithEnvRow, error) {
	row := q.db.QueryRow(ctx, getTemplateBuildWithEnv, arg.TemplateID, arg.BuildID)
	var i GetTemplateBuildWithEnvRow
	err := row.Scan(
		&i.Env.ID,
		&i.Env.CreatedAt,
		&i.Env.UpdatedAt,
