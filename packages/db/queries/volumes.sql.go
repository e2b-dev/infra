// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.30.0
// source: volumes.sql

package queries

import (
	"context"

	"github.com/google/uuid"
)

const createVolume = `-- name: CreateVolume :one
INSERT INTO volumes (team_id, volume_type, name, volume_path)
VALUES ($1, $2, $3, $4)
RETURNING id, team_id, name, volume_type, created_at, volume_path
`

type CreateVolumeParams struct {
	TeamID     uuid.UUID
	VolumeType string
	Name       string
	VolumePath *string
}

func (q *Queries) CreateVolume(ctx context.Context, arg CreateVolumeParams) (Volume, error) {
	row := q.db.QueryRow(ctx, createVolume,
		arg.TeamID,
		arg.VolumeType,
		arg.Name,
		arg.VolumePath,
	)
	var i Volume
	err := row.Scan(
		&i.ID,
		&i.TeamID,
		&i.Name,
		&i.VolumeType,
		&i.CreatedAt,
		&i.VolumePath,
	)
	return i, err
}

const deleteVolume = `-- name: DeleteVolume :exec
DELETE FROM volumes WHERE team_id = $1 AND id = $2
`

type DeleteVolumeParams struct {
	TeamID   uuid.UUID
	VolumeID uuid.UUID
}

func (q *Queries) DeleteVolume(ctx context.Context, arg DeleteVolumeParams) error {
	_, err := q.db.Exec(ctx, deleteVolume, arg.TeamID, arg.VolumeID)
	return err
}

const findVolumesByTeamID = `-- name: FindVolumesByTeamID :many
SELECT id, team_id, name, volume_type, created_at, volume_path FROM volumes WHERE team_id = $1
`

func (q *Queries) FindVolumesByTeamID(ctx context.Context, teamID uuid.UUID) ([]Volume, error) {
	rows, err := q.db.Query(ctx, findVolumesByTeamID, teamID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []Volume
	for rows.Next() {
		var i Volume
		if err := rows.Scan(
			&i.ID,
			&i.TeamID,
			&i.Name,
			&i.VolumeType,
			&i.CreatedAt,
			&i.VolumePath,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const getVolume = `-- name: GetVolume :one
SELECT id, team_id, name, volume_type, created_at, volume_path FROM volumes WHERE id = $1 AND team_id = $2
`

type GetVolumeParams struct {
	VolumeID uuid.UUID
	TeamID   uuid.UUID
}

func (q *Queries) GetVolume(ctx context.Context, arg GetVolumeParams) (Volume, error) {
	row := q.db.QueryRow(ctx, getVolume, arg.VolumeID, arg.TeamID)
	var i Volume
	err := row.Scan(
		&i.ID,
		&i.TeamID,
		&i.Name,
		&i.VolumeType,
		&i.CreatedAt,
		&i.VolumePath,
	)
	return i, err
}

const getVolumesByName = `-- name: GetVolumesByName :many
SELECT id, team_id, name, volume_type, created_at, volume_path FROM volumes WHERE team_id = $1 AND name IN (
    SELECT UNNEST($2::text[])
)
`

type GetVolumesByNameParams struct {
	TeamID      uuid.UUID
	VolumeNames []string
}

func (q *Queries) GetVolumesByName(ctx context.Context, arg GetVolumesByNameParams) ([]Volume, error) {
	rows, err := q.db.Query(ctx, getVolumesByName, arg.TeamID, arg.VolumeNames)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []Volume
	for rows.Next() {
		var i Volume
		if err := rows.Scan(
			&i.ID,
			&i.TeamID,
			&i.Name,
			&i.VolumeType,
			&i.CreatedAt,
			&i.VolumePath,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const updateVolumePath = `-- name: UpdateVolumePath :exec
UPDATE volumes SET volume_path = $1 WHERE id = $2 AND team_id = $3
`

type UpdateVolumePathParams struct {
	VolumePath *string
	VolumeID   uuid.UUID
	TeamID     uuid.UUID
}

func (q *Queries) UpdateVolumePath(ctx context.Context, arg UpdateVolumePathParams) error {
	_, err := q.db.Exec(ctx, updateVolumePath, arg.VolumePath, arg.VolumeID, arg.TeamID)
	return err
}
