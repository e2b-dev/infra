...
func TestGetTemplateByAlias_MatchesNamespace(t *testing.T) {
	...
	result, err := db.GetTemplateByAlias(ctx, queries.GetTemplateByAliasParams{
		Alias:     "my-template",
		Namespace: &teamSlug,
	})
...
func TestGetTemplateByAlias_NotFound(t *testing.T) {
	...
	_, err := db.GetTemplateByAlias(ctx, queries.GetTemplateByAliasParams{
		Alias:     "team-scoped",
...
func TestGetTemplateById(t *testing.T) {
	...
	result, err := db.GetTemplateById(ctx, templateID)
	require.NoError(t, err)
	assert.Equal(t, templateID, result.ID)
}
