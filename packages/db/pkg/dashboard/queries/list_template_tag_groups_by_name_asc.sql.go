...
// versions:
//   sqlc v1.29.0
// source: list_template_tag_groups_by_name_asc.sql
...
const listTemplateTagGroupsByNameAsc = `-- name: ListTemplateTagGroupsByNameAsc :many
WITH tag_window AS (
    SELECT
        eba.tag,
...
type ListTemplateTagGroupsByNameAscParams struct {
...
type ListTemplateTagGroupsByNameAscRow struct {
...
func (q *Queries) ListTemplateTagGroupsByNameAsc(ctx context.Context, arg ListTemplateTagGroupsByNameAscParams) ([]ListTemplateTagGroupsByNameAscRow, error) {
	rows, err := q.db.Query(ctx, listTemplateTagGroupsByNameAsc,
	...
	defer rows.Close()
	var items []ListTemplateTagGroupsByNameAscRow
	for rows.Next() {
		var i ListTemplateTagGroupsByNameAscRow
		if err := rows.Scan(
			&i.AssignmentID,
			&i.Tag,
			&i.BuildID,
