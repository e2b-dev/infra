...
WHERE
    e.id = $1::text
    AND (
        (e.team_id = $2::uuid AND e.source = 'template')
        OR ($3::boolean AND d.env_id IS NOT NULL)
    )
LIMIT 1
`
...
func (q *Queries) GetTeamTemplate(ctx context.Context, arg GetTeamTemplateParams) (GetTeamTemplateRow, error) {
	row := q.db.QueryRow(ctx, getTeamTemplate, arg.TemplateID, arg.TeamID, arg.IncludeDefaults)
