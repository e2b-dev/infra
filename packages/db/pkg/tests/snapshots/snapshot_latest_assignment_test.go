...
func TestGetLastSnapshot_ReturnsLatestAssignment(t *testing.T) {
	...
	baseTemplateID := testutils2.CreateTestTemplate(t, db, teamID)
	...
	result1 := testutils2.UpsertTestSnapshot(t, ctx, db, snapshotTemplateID, sandboxID, teamID, baseTemplateID)
	...
	result2 := testutils2.UpsertTestSnapshot(t, ctx, db, snapshotTemplateID, sandboxID, teamID, baseTemplateID)
...
func TestGetLastSnapshot_OnlyReturnsSuccessBuilds(t *testing.T) {
	...
	testutils2.UpsertTestSnapshotWithStatus(t, ctx, db, snapshotTemplateID, sandboxID, teamID, baseTemplateID, "snapshotting")
...
func TestGetSnapshotsWithCursor_ReturnsLatestAssignment(t *testing.T) {
	...
	testutils2.UpsertTestSnapshot(t, ctx, db, snapshotTemplateID, sandboxID, teamID, baseTemplateID)
...
func TestGetLastSnapshot_BuildSharedWithOtherTemplate(t *testing.T) {
	...
	otherTemplateID := testutils2.CreateTestTemplate(t, db, teamID)
	time.Sleep(10 * time.Millisecond)
	testutils2.CreateTestBuildAssignment(t, ctx, db, otherTemplateID, build1ID, "default")
...
func TestGetLastSnapshot_IgnoresNonDefaultTags(t *testing.T) {
	...
	otherBuildID := testutils2.CreateTestBuild(t, ctx, db, result1.TemplateID, "success")
	// Delete the auto-created 'default' assignment from the trigger so the build only has 'v1' tag
	testutils2.DeleteTriggerBuildAssignment(t, ctx, db, result1.TemplateID, otherBuildID, "default")
	testutils2.CreateTestBuildAssignment(t, ctx, db, result1.TemplateID, otherBuildID, "v1")
...
func TestGetLastSnapshot_AssignmentOrderDifferentFromBuildOrder(t *testing.T) {
	...
	teamID := testutils2.CreateTestTeam(t, db)
	baseTemplateID := testutils2.CreateTestTemplate(t, db, teamID)
	snapshotTemplateID := testutils2.CreateTestTemplate(t, db, teamID)

	// Create builds: build1 (older), build2 (newer)
	build1ID := testutils2.CreateTestBuild(t, ctx, db, snapshotTemplateID, "success")
	time.Sleep(10 * time.Millisecond)
	build2ID := testutils2.CreateTestBuild(t, ctx, db, snapshotTemplateID, "success")

	// Assign in REVERSE order: build2 first, then build1
	// Even though build2 was created later, build1 has the latest assignment
	testutils2.CreateTestBuildAssignment(t, ctx, db, snapshotTemplateID, build2ID, "default")
	time.Sleep(10 * time.Millisecond)
	testutils2.CreateTestBuildAssignment(t, ctx, db, snapshotTemplateID, build1ID, "default")

	// Create snapshot record
	sandboxID := "sandbox-" + uuid.New().String()
	testutils2.CreateSnapshotRecord(t, ctx, db, snapshotTemplateID, sandboxID, teamID, baseTemplateID)
