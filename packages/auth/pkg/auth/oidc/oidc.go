...
// oidcHTTPTimout is the timeout used for OIDC discovery and JWKS HTTP
// requests.
const oidcHTTPTimout = 10 * time.Second
...
func NewVerifier(ctx context.Context, entry Entry, httpClient *http.Client) (*Verifier, error) {
	...
	return &Verifier{
		keyfunc:     keyFunc,
		userIDClaim: jwtutil.DefaultUserIDClaim,
...
func fetchDiscoveryDocument(ctx context.Context, httpClient *http.Client, discoveryURL string) (*discoveryDocument, error) {
	fetchCtx, cancel := context.WithTimeout(ctx, oidcHTTPTimout)
	defer cancel()
