...
type ProviderConfig struct {
	...
	Legacy *legacy.Entry `json:"legacy"`
}
...
func NewVerifier(ctx context.Context, config ProviderConfig, oidcHTTPClient *http.Client) (*Verifier, error) {
	...
	for i, entry := range normalized.JWT {
		s, err := oidc.NewVerifier(ctx, entry, oidcHTTPClient)
		if err != nil {
