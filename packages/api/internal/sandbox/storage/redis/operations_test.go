...
func TestStartRemoving_CallbackDeletesTransitionKey(t *testing.T) {
	t.Parallel()

	storage, client := setupTestStorage(t)
	ctx := context.Background()

	sbx := createTestSandbox("callback-delete")
	err := storage.Add(ctx, sbx)
	require.NoError(t, err)
...
func TestStartRemoving_CallbackSetsErrorOnFailure(t *testing.T) {
	...
	// Transition key should still exist with JSON error format
	value, err := client.Get(ctx, transitionKey).Result()
	require.NoError(t, err)
	assert.Contains(t, value, `"error":`)
	assert.Contains(t, value, "test error")
}
...
func TestStartRemoving_ErrorPropagation(t *testing.T) {
	t.Parallel()

	storage, _ := setupTestStorage(t)
	ctx := context.Background()

	sbx := createTestSandbox("error-propagation")
	err := storage.Add(ctx, sbx)
	require.NoError(t, err)
	...
	case <-done:
		require.Error(t, waitErr)
		assert.Contains(t, waitErr.Error(), "sandbox is in failed state")
		assert.Contains(t, waitErr.Error(), "connection refused")
	case <-time.After(time.Second):
		t.Fatal("WaitForStateChange did not complete in time")
	}

	// After error key TTL expires, retry should be allowed
	// Wait for TTL to expire (errorKeyTTL is 5 seconds + buffer)
