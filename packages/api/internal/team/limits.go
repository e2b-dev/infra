...
// LimitResourcesWithAddons validates and limits resources based on tier + addons
func LimitResourcesWithAddons(tierWithAddons *TierWithAddons, cpuCount, memoryMB *int32) (int64, int64, *api.APIError) {
	cpu := constants.DefaultTemplateCPU
	ramMB := constants.DefaultTemplateMemory

	maxVcpu := tierWithAddons.MaxVcpu()
	maxRamMb := tierWithAddons.MaxRamMb()
	...
	if cpuCount != nil {
		...
		if cpu > maxVcpu {
			return 0, 0, &api.APIError{
				Err:       fmt.Errorf("CPU count exceeds team limits (%d)", maxVcpu),
				ClientMsg: fmt.Sprintf("CPU count can't be higher than %d (if you need to increase this limit, please contact support)", maxVcpu),
				Code:      http.StatusBadRequest,
			}
		}
	}
	...
	if memoryMB != nil {
		...
		if ramMB > maxRamMb {
			return 0, 0, &api.APIError{
				Err:       fmt.Errorf("memory exceeds team limits (%d MiB)", maxRamMb),
				ClientMsg: fmt.Sprintf("Memory can't be higher than %d MiB (if you need to increase this limit, please contact support)", maxRamMb),
				Code:      http.StatusBadRequest,
			}
		}
	}
