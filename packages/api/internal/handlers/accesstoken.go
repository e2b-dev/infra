...
func (a *APIStore) PostAccessTokens(c *gin.Context) {
	...
	if a.featureFlags.BoolFlag(ctx, featureflags.DisableAccessTokenIssuanceFlag, featureflags.UserContext(userID.String())) {
		a.sendAPIStoreError(c, http.StatusForbidden, "Creating new access tokens is disabled. E2B_ACCESS_TOKEN is deprecated; use an API key (E2B_API_KEY) instead. See https://e2b.dev/docs/migration/access-token-deprecation")
