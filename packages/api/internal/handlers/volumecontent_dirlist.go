...
func (a *APIStore) GetVolumesVolumeIDDir(c *gin.Context, volumeID api.VolumeID, params api.GetVolumesVolumeIDDirParams) {
	a.executeOnOrchestrator(c, func(ctx context.Context, client *clusters.GRPCClient) error {
		...
		files := make([]api.VolumeDirectoryItem, 0, len(response.GetFiles()))
		for _, item := range response.GetFiles() {
			files = append(files, api.VolumeDirectoryItem{
				Ctime:       item.GetCtime().AsTime(),
				IsDirectory: item.GetIsDirectory(),
				Mode:        item.GetMode(),
				Mtime:       item.GetMtime().AsTime(),
				Name:        item.GetName(),
				Size:        item.GetSize(),
			})
		}
