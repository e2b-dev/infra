...
func (o *Orchestrator) KeepAliveFor(ctx context.Context, teamID uuid.UUID, sandboxID string, duration time.Duration, allowShorter bool, persistTimeout bool) *api.APIError {
	...
	var timeoutSecondsToPersist *int32
	if persistTimeout {
		timeoutSecondsToPersist = sbx.SandboxTimeoutSeconds
	}
	err = o.UpdateSandbox(ctx, sandboxID, sbx.EndTime, timeoutSecondsToPersist, sbx.ClusterID, sbx.NodeID)
	if err != nil {
