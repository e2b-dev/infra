...
func (a *placementAffinity) record(ctx context.Context, cfg placementAffinityConfig, clusterID uuid.UUID, nodeID, buildID string) {
	if a == nil || nodeID == "" || buildID == "" || cfg.buildWeight == 0 {
		return
	}
...
func (a *placementAffinity) scores(ctx context.Context, cfg placementAffinityConfig, clusterID uuid.UUID, buildID string) map[string]float64 {
	if a == nil || buildID == "" || cfg.buildWeight == 0 {
		return nil
	}
