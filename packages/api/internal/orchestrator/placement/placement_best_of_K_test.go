...
func TestBestOfK_PowerOfKChoices(t *testing.T) {
	...
	config := BestOfKConfig{
		R:     10,
		Alpha: 0.5,
		K:     3, // Sample only 3 nodes out of many
	}
	algo := NewBestOfK(config).(*BestOfK)
	...
	// If we had successful selections, check distribution
	if successCount > 0 {
		...
		// If no selections succeeded, that's OK for this test
		t.Skip("No successful node selections - nodes likely have 0 CPU count")
	}
}

func TestBestOfK_InterfaceCompliance(t *testing.T) {
	// This test ensures that BestOfK implements the Algorithm interface
