...
func (o *Orchestrator) reportLongRunningSandboxes(ctx context.Context) {
	...
	for {
		...
		case <-ctx.Done():
			...
			return
		case <-ticker.C:
			sandboxes := o.sandboxStore.Items(ctx, nil)
			longRunningSandboxes := make([]*store.Sandbox, 0, len(sandboxes))
...
func sendAnalyticsForLongRunningSandboxes(ctx context.Context, analytics *analyticscollector.Analytics, instances []*store.Sandbox) {
	if len(instances) == 0 {
...
func (o *Orchestrator) analyticsRemove(ctx context.Context, sandbox *store.Sandbox, removeType store.RemoveType) {
...
func (o *Orchestrator) analyticsInsert(ctx context.Context, sandbox *store.Sandbox, created bool) {
	ctx, cancel := context.WithTimeout(ctx, reportTimeout)
	defer cancel()
...
func (o *Orchestrator) countersInsert(ctx context.Context, sandbox *store.Sandbox, newlyCreated bool) {
...
func (o *Orchestrator) countersRemove(ctx context.Context, sandbox *store.Sandbox, _ store.RemoveType) {
	attributes := []attribute.KeyValue{
		telemetry.WithTeamID(sandbox.TeamID.String()),
	}
