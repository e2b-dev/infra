...
func TestCreationMetadata_FieldsArePassedToCallback(t *testing.T) {
	t.Parallel()
	...
	header := http.Header{}
	header.Set("lang", "python")
	header.Set("publisher", "e2b")

	mcp := []string{"my-server-1", "my-server-2"}
	requestStart := now.Add(-time.Second)
	...
	got := ec.getMetas()[0]
	assert.True(t, got.IsResume)
	assert.Equal(t, "carrot-team", got.TeamName)
	assert.Equal(t, "python", got.RequestHeader.Get("lang"))
	assert.Equal(t, "e2b", got.RequestHeader.Get("publisher"))
	assert.Equal(t, mcp, got.MCPServerNames)
	assert.True(t, got.RequestStartTime.Equal(requestStart))
}
