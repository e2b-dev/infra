...
func (tm *TemplateManager) BuildStatusSync(ctx context.Context, buildID uuid.UUID, templateID string, clusterId *uuid.UUID, clusterNodeId *string) {
	...
	placement, err := tm.getPlacement(clusterId, clusterNodeId)
	if err != nil {
		logger.Error("Error when getting build placement", zap.Error(err))
		return
	}
...
func (c *PollBuildStatus) setStatus(ctx context.Context) error {
	status, err := c.placement.GetStatus(ctx, c.buildID.String(), c.templateID)
