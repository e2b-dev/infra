...
// TestConnect_SubscriberServedRetainedOnEndClose covers the reaper→Connect
// contract that the Wait fix restores for fresh processes: a Connect that finds
// the process still live and subscribes, but whose terminal event was already
// fanned out before it subscribed, must be served the retained exit the instant
// EndEvent closes — not block forever. (Wait now closes EndEvent, mirroring the
// re-adopt reaper; here the close is driven directly so the test needs no root.)
func TestConnect_SubscriberServedRetainedOnEndClose(t *testing.T) {
	t.Parallel()

	client, svc, cleanup := newRetentionTestService(t)
	defer cleanup()

	const pid = uint32(7373)
	logger := zerolog.Nop()
	h := handler.Readopt(handler.ReadoptArgs{Pid: pid}, &logger)
	svc.processes.Store(pid, h) // live when Connect subscribes
	svc.terminated.Store(pid, &retainedExit{
		pid: pid,
		end: &rpc.ProcessEvent_EndEvent{Exited: true, ExitCode: 9, Status: "exited"},
	})

	// The event was fanned out before this Connect subscribed; the close (what
	// Wait now does) is what makes the retention fallback fire instead of hanging.
	go func() {
	...
	require.NotNil(t, last.GetEnd(), "must serve the retained End on channel close, not hang")
	assert.Equal(t, int32(9), last.GetEnd().GetExitCode())
}
