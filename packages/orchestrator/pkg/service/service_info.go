...
func (s *Server) ServiceStatusOverride(ctx context.Context, req *orchestratorinfo.ServiceStatusChangeRequest) (*emptypb.Empty, error) {
	...
	currentStatus := s.info.GetStatus()
	if (s.drainStarted.Load() || currentStatus == orchestratorinfo.ServiceInfoStatus_Draining) &&
		(requestedStatus == orchestratorinfo.ServiceInfoStatus_Healthy || requestedStatus == orchestratorinfo.ServiceInfoStatus_Standby) {
		return nil, status.Error(codes.FailedPrecondition, "service drain cannot be reversed")
	}

	if requestedStatus == orchestratorinfo.ServiceInfoStatus_Draining {
		s.drainStarted.Store(true)
		if s.drainController != nil {
			s.drainController.StartDraining(ctx)
			if req.GetForceStop() {
				forceCtx := context.WithoutCancel(ctx)
				go func() {
					if err := s.drainController.ForceStop(forceCtx); err != nil {
						logger.L().Error(forceCtx, "forced drain failed", zap.Error(err))
					}
				}()
			}
		}
	...
	s.info.SetStatus(ctx, requestedStatus)
