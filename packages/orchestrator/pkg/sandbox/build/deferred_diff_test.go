...
func TestDeferredDiff_DelegatesAfterResolve(t *testing.T) {
	...
	materialized := makeLocalDiff(t, blockSize, data)
	t.Cleanup(func() { _ = materialized.Close() })

	inner := utils.NewSetOnce[Diff]()
	d := NewDeferredDiff(materialized.CacheKey(), blockSize, inner)

	// Resolve on a goroutine; the reads below block until it lands.
	go func() { _ = inner.SetValue(materialized) }()
