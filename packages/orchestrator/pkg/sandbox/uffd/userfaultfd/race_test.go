	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			withRaceContext(t, func(ctx context.Context) {
				...
				// Planting goes through testConfig.sourcePatcher so
				// it lands BOTH in the parent's MemorySlicer and in
				// the on-disk content file the child reads.
				...
				token, err := h.installFaultBarrier(ctx, addr, barrierBeforeRLock)
				require.NoError(t, err)
				...
				// RLock.
				madviseCtx, madviseCancel := context.WithTimeout(ctx, madviseBudget)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			withRaceContext(t, func(ctx context.Context) {
				...
				token, err := h.installFaultBarrier(ctx, addr, barrierBeforeFaultPage)
				require.NoError(t, err)
...
func TestFaultedShortCircuitOrdering(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			withRaceContext(t, func(_ context.Context) {
				cfg := testConfig{
					pagesize:      tt.pagesize,
					numberOfPages: 2,
					gated:         true,
