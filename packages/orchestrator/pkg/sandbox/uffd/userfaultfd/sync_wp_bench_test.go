...
// Phase-0 spike for the always-sync memory-CoW design
// (~/workspace/designs/in-place-memory-cow-snapshot.md §8.1).
//
// Question it answers: with UFFD_FEATURE_WP_ASYNC *dropped*, what is the raw
// per-fault round-trip cost of a SYNCHRONOUS write-protect fault on a 2 MiB
// hugepage — i.e. the steady-state tax the "always-sync WP" design would put
// on every guest write to a (re-)protected hugepage. It also measures the
// per-round re-arm (write-protect the whole range) cost.
//
...
// and resolves each WP fault with UFFDIO_WRITEPROTECT(unprotect)+wake (the
// "orchestrator handler", tracking-only fast path — no copy). Because the
// resolver is a separate thread, a store blocked in the WP fault is always
// unblocked externally; we additionally disable GC during the measurement so
// no STW can wait on the fault-blocked M.
//
// This deliberately does NOT exercise the production serve loop (which today
// treats WP faults as fatal, userfaultfd.go:412) — it measures the kernel +
// minimal-handler floor, which is the number the go/no-go rides on.
//
...
func hugepageMmap(tb testing.TB, size uint64) ([]byte, uintptr) {
	...
	b, err := syscall.Mmap(
		...
		syscall.MAP_PRIVATE|syscall.MAP_ANONYMOUS|unix_MAP_HUGETLB|unix_MAP_HUGE_2MB,
	)
	if err != nil {
...
const (
	unix_MAP_HUGETLB  = 0x40000
	unix_MAP_HUGE_2MB = 21 << 26
)

// wpRange write-protects (mode=UFFDIO_WRITEPROTECT_MODE_WP) or unprotects
// (mode=0) [addr, addr+length) in one ioctl, without the single-page address
// masking that Fd.writeProtect applies — used to arm the whole range at once.
func wpRange(fd Fd, addr, length uintptr, mode CULong) error {
	wp := newUffdioWriteProtect(CULong(addr), CULong(length), mode)
	if _, _, errno := syscall.Syscall(syscall.SYS_IOCTL, uintptr(fd), UFFDIO_WRITEPROTECT, uintptr(unsafe.Pointer(&wp))); errno != 0 {
		return errno
	}
...
func TestSyncWPFaultLatency(t *testing.T) {
	...
	if nRounds < 2 {
		nRounds = 2 // need >=1 warmup + >=1 measured round
	}
	...
	go func() {
		...
		for resolved < totalFaults {
			...
			if werr := fd.writeProtect(addr, uintptr(pagesize), 0); werr != nil {
	...
	for r := 0; r < nRounds; r++ {
		...
		armStart := time.Now()
		if err := wpRange(fd, memStart, uintptr(size), UFFDIO_WRITEPROTECT_MODE_WP); err != nil {
			t.Fatalf("arm wpRange (round %d): %v", r, err)
		}
	...
	for k := 0; k < handlers; k++ {
		...
		go func() {
			...
			for addr := range workCh {
				_ = fd.writeProtect(addr, uintptr(pagesize), 0) // unprotect + wake
			}
		}()
	}
	...
	for w := 0; w < W; w++ {
		...
		go func(w, lo, hi int) {
			...
			for r := 0; r < nRounds; r++ {
				...
				if err := wpRange(fd, rangeStart, uintptr(uint64(hi-lo)*pagesize), UFFDIO_WRITEPROTECT_MODE_WP); err != nil {
...
// handler keeps per-fault latency bounded under concurrent load — the Phase-0.5
