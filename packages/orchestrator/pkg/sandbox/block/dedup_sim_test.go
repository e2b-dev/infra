...
func TestDedupFragmentationSim(t *testing.T) {
	...
	t.Logf("== global budget sweep (pages/pause) ==")
	logSimHeader(t)
	for _, budget := range []int{0, 512, 2048, 8192} {
		cfg := base
		cfg.globalBudget = budget
		s := simScenario{name: fmt.Sprintf("dedup+global=%d", budget), planner: plannerDedup(DedupBudget{MaxPromotedParentPagesTotal: budget})}
	...
	for _, scattered := range []int{20, 60, 200} {
		for _, cycles := range []int{12, 24} {
			...
			for _, s := range []simScenario{
				...
				{name: fmt.Sprintf("s=%d c=%d +global", scattered, cycles), planner: plannerDedup(DedupBudget{MaxPromotedParentPagesTotal: cfg.globalBudget})},
			} {
				logSimRow(t, runSimChain(t, cfg, s))
			}
		}
...
func simScenarios(cfg simConfig) []simScenario {
	return []simScenario{
		...
		{"dedup+global", plannerDedup(DedupBudget{MaxPromotedParentPagesTotal: cfg.globalBudget})},
		{"dedup+global+block", plannerDedup(DedupBudget{
			MaxPromotedParentPagesTotal:    cfg.globalBudget,
			MaxFetchWindowsPerBlock:        8,
			MaxPromotedParentPagesPerBlock: 64,
		})},
		{"dedup+runKnapsack", plannerRunKnapsack(cfg.globalBudget)},
		{"dedup+global+flat6", plannerFlatten(6, plannerDedup(DedupBudget{MaxPromotedParentPagesTotal: cfg.globalBudget}))},
	}
}
