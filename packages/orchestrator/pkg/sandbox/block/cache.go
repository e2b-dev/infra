...
func (c *Cache) copyFromMemfd(ctx context.Context, memfd *Memfd, ranges []Range) error {
	...
	for _, r := range ranges {
		...
		for remaining > 0 {
			n, err := syscall.Pread(memfd.memfd, (*c.mmap)[cacheOff:cacheOff+remaining], srcOff)
		...
		// Free the hugepage now that the data is in the cache file.
		if err := memfd.FreePages(r.Start, r.Size); err != nil {
			logger.L().Warn(ctx, "failed to punch hole in memfd", zap.Error(err))
		}
