...
// TestOverlaySwapCache_ClosedSealingPrefersFoldedCache pins the fold-flavor
// contract of the closed-sealing fall-through: when a fold moved the sealing
// cache's blocks into the WRITABLE cache before closing it, a reader must
// resolve those blocks from the cache — never from the base device, whose
// copy is the stale pre-swap template content. (The mid-read interleaving —
// cache miss, then fold+close, then the sealing read observing
// CacheClosedError — is what ReadAt's re-read-the-cache retry exists for;
// it cannot be constructed statically, so this test pins the end state and
// the retry is exercised by the ordering argument in isCacheClosed's doc.)
func TestOverlaySwapCache_ClosedSealingPrefersFoldedCache(t *testing.T) {
	t.Parallel()

	blockSize := int64(header.PageSize)

	base := &fakeOriginalDevice{data: fill(blockSize, 0xCC)}

	c0, err := NewCache(blockSize, blockSize, t.TempDir()+"/c0", false)
	require.NoError(t, err)
	o := NewOverlay(base, c0)

	// Pre-swap dirty block, then swap: c0 becomes sealing.
	_, err = o.WriteAt(fill(blockSize, 0xAA), 0)
	require.NoError(t, err)
	c1, err := NewCache(blockSize, blockSize, t.TempDir()+"/c1", false)
	require.NoError(t, err)
	...
	// Fold (moves 0xAA into c1), then close the detached sealing cache.
	detached, err := o.FoldSealing()
	require.NoError(t, err)
	require.NoError(t, detached.Close())
	...
	_, err = o.ReadAt(t.Context(), buf, 0)
	require.NoError(t, err)
	require.Equal(t, fill(blockSize, 0xAA), buf,
		"pre-swap dirty block must resolve from the folded writable cache, not the base")
