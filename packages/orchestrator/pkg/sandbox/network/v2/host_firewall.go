...
func NewHostFirewall(defaultGw string, config network.Config) (_ *HostFirewall, err error) {
	metrics, err := NewMetrics(otel.GetMeterProvider())
	if err != nil {
		return nil, fmt.Errorf("create host firewall metrics: %w", err)
	}

	return NewHostFirewallWithMetrics(defaultGw, config, metrics)
}

// NewHostFirewallWithMetrics creates a host firewall with explicitly scoped
// metrics, avoiding the global OpenTelemetry provider in production and tests.
// A nil metrics argument disables host firewall failure metrics.
func NewHostFirewallWithMetrics(defaultGw string, config network.Config, metrics *Metrics) (_ *HostFirewall, err error) {
...
func (hf *HostFirewall) ReconcileSlots(ctx context.Context, activeSlots []*SlotV2) (err error) {
	...
	defer hf.resetConnOnError(ctx, operationReconcile, true, &err)
...
func (hf *HostFirewall) AddSlot(ctx context.Context, slotV2 *SlotV2) (err error) {
	...
	defer hf.resetConnOnError(ctx, operationAddSlot, false, &err)
...
func (hf *HostFirewall) RemoveSlot(ctx context.Context, slotV2 *SlotV2) (err error) {
	...
	defer hf.resetConnOnError(ctx, operationRemoveSlot, false, &err)
...
// PurgeHostFirewallTable idempotently removes the v2-host-firewall table (v1-path rollback cleanup).
func PurgeHostFirewallTable() (err error) {
	conn, err := nftables.New(nftables.AsLasting())
	if err != nil {
		return fmt.Errorf("new nftables conn: %w", err)
	}
	defer func() { err = errors.Join(err, conn.CloseLasting()) }()

	tables, err := conn.ListTablesOfFamily(nftables.TableFamilyINet)
	if err != nil {
		return fmt.Errorf("list inet tables: %w", err)
	}
	for _, t := range tables {
		if t.Name == hostFwTableName {
			conn.DelTable(t)
			if err := conn.Flush(); err != nil {
				return fmt.Errorf("delete %s table: %w", hostFwTableName, err)
			}
...
func (hf *HostFirewall) resetConnOnError(ctx context.Context, operation string, reconciliation bool, err *error) {
	if *err == nil {
		return
	}

	if hf.metrics == nil {
		joinOperationError(err, hf.reset(ctx))

		return
	}

	resetErr := hf.metrics.recordHostFirewallFailure(ctx, operation, reconciliation, func() error {
		return hf.reset(ctx)
	})
	joinOperationError(err, resetErr)
}
