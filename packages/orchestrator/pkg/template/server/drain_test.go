...
func TestWaitCanceledContextCancelsBuildsAndReturns(t *testing.T) {
	...
	case <-time.After(time.Second):
		t.Fatal("Wait did not cancel running build after build start gate drained")
	}
	...
	case err := <-waitErr:
		require.ErrorIs(t, err, context.Canceled)
	case <-time.After(time.Second):
		t.Fatal("Wait blocked on active build after forced cancellation")
	}
}

func TestForcedWaitCancelsBuildCreatedDuringBuildStartDrain(t *testing.T) {
	t.Parallel()
	...
	case <-s.drainDone:
	case <-time.After(time.Second):
		t.Fatal("Wait did not start draining")
	}

	select {
	case <-sentinelBuild.Result.Done:
	case <-time.After(time.Second):
		t.Fatal("Wait did not run the first forced cancellation pass")
	}

	select {
	case err := <-waitErr:
		t.Fatalf("Wait returned before in-flight build start finished: %v", err)
	default:
	}
	...
	case <-buildInfo.Result.Done:
		...
		require.Equal(t, builderrors.ErrCanceled.Error(), result.Reason.GetMessage())
	case <-time.After(time.Second):
		t.Fatal("Wait did not cancel build created during build start drain")
	}
	...
	case err := <-waitErr:
		require.NoError(t, err)
	case <-time.After(time.Second):
		t.Fatal("Wait did not return after build finished")
	}
}
