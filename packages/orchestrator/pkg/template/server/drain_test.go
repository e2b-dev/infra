...
func TestWaitCanceledContextCancelsBuildsAndReturns(t *testing.T) {
	...
	case <-time.After(time.Second):
		t.Fatal("ForceStop did not cancel running build after template operation start gate drained")
	}
...
func TestForceStopCancelsBuildCreatedDuringTemplateOperationStartDrain(t *testing.T) {
	t.Parallel()
	...
	select {
	case err := <-waitErr:
		t.Fatalf("ForceStop returned before in-flight template operation start finished: %v", err)
	default:
	}
	...
	select {
	case <-buildInfo.Result.Done:
		t.Fatal("late build was canceled before template operation start drain completed")
	default:
	}
	...
	case <-buildInfo.Result.Done:
		...
		require.Equal(t, builderrors.ErrCanceled.Error(), result.Reason.GetMessage())
	case <-time.After(time.Second):
		t.Fatal("ForceStop did not cancel build created during template operation start drain")
	}
