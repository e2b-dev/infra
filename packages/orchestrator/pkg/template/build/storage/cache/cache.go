...
var tracer = otel.Tracer("github.com/e2b-dev/infra/packages/orchestrator/pkg/template/build/storage/cache")
...
func (h *HashIndex) LayerMetaFromHash(ctx context.Context, hash string) (LayerMetadata, error) {
	...
	hashPath, err := paths.HashToPath(h.cacheScope, hash)
	if err != nil {
		return LayerMetadata{}, fmt.Errorf("invalid hash index path: %w", err)
	}

	obj, err := h.indexStorage.OpenBlob(ctx, hashPath, storage.LayerMetadataObjectType)
	if err != nil {
...
func (h *HashIndex) SaveLayerMeta(ctx context.Context, hash string, template LayerMetadata) error {
	...
	hashPath, err := paths.HashToPath(h.cacheScope, hash)
	if err != nil {
		return fmt.Errorf("invalid hash index path: %w", err)
	}
