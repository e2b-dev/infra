...
func (c *Copy) Execute(
	...
	layerFilesPath, err := paths.GetLayerFilesCachePath(c.CacheScope, step.GetFilesHash())
	if err != nil {
		return metadata.Context{}, fmt.Errorf("invalid layer files cache path: %w", err)
	}

	obj, err := c.FilesStorage.OpenBlob(ctx, layerFilesPath, storage.BuildLayerFileObjectType)
	if err != nil {
