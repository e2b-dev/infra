...
func (h *APIStore) Logs(c *gin.Context) {
	...
	if err := h.forwardLogs(c.Request.Context(), logs); err != nil {
...
func (h *APIStore) forwardLogs(ctx context.Context, payload []byte) error {
	request, err := http.NewRequestWithContext(ctx, http.MethodPost, h.collectorAddr, bytes.NewBuffer(payload))
	if err != nil {
