...
// CLICKHOUSE_CONNECTION_STRING. Order from the env var is preserved; first
// occurrence wins on dedup. Any entry that exactly matches (after TrimSpace)
// the singular DSN, or a previously-kept entry, is dropped and returned in
// droppedDuplicates so the caller can surface it in logs. Returns nil
// endpoints if nothing remains.
