...
import (
	...
	"github.com/e2b-dev/infra/packages/orchestrator/internal/sandbox/disk/nbd"
...
func (d *DeviceWithClose) ReadAt(ctx context.Context, p []byte, off int64) (int, error) {
	return d.File.ReadAt(ctx, p, off)
}
...
func executeForNbd(
	...
	mnt := nbd.NewDirectPathMount(overlay, nil, zap.NewNop())
...
func checkNbd(t *testing.T, ctx context.Context) error {
	...
	err = executeForNbd(t, ctx, baseOverlay, func(mountedPath string) error {
		...
		err = os.WriteFile(contentPath, []byte(content), 0o644)
		if err != nil {
