	...
	tests := []struct {
	...
	}{
		{
			...
			expectedScriptContent: []string{
				...
				fmt.Sprintf("ip netns exec ns-789 /fc-versions/1.4.0/%s/firecracker --api-sock", utils.TargetArch()),
		...
		},
		{
			...
			expectedScriptContent: []string{
				...
				fmt.Sprintf("ip netns exec legacy-ns /fc-versions/1.3.0/%s/firecracker --api-sock", utils.TargetArch()),
		...
		},
		{
			...
			expectedScriptContent: []string{
				...
				fmt.Sprintf("ip netns exec custom-ns-id /fc-versions/1.5.0-beta/%s/firecracker --api-sock", utils.TargetArch()),
