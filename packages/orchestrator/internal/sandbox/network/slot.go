...
func (s *Slot) ConfigureInternet(ctx context.Context, egress sandbox_network.Egress) (e error) {
	_, span := tracer.Start(ctx, "slot-internet-configure", trace.WithAttributes(
		attribute.String("namespace_id", s.NamespaceID()),
	))
	defer span.End()

	if egress.NoFirewallRules() {
		return nil
	}
	...
	err = n.Do(func(_ ns.NetNS) error {
		return s.Firewall.ReplaceUserRules(egress.Allowed.CIDRs(), egress.Denied.CIDRs())
	})
	if err != nil {
...
func (s *Slot) UpdateInternet(ctx context.Context, egress sandbox_network.Egress) error {
