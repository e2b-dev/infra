...
func Handle(conn net.Conn, exports []*Export, options *Options) error {
	...
	log.Printf("[Server] Handling connection from %s\n", conn.LocalAddr())
	// Negotiation
	...
	log.Printf("[Server] Sent negotiation header\n")
	_, err := io.CopyN(io.Discard, conn, 4) // Discard client flags (uint32)
	if err != nil {
		return err
	}

	log.Printf("[Server] Discarded client flags\n")
	var export *Export
n:
	for {
		log.Printf("[Server] Waiting for option\n")
		...
		case protocol.NEGOTIATION_ID_OPTION_INFO, protocol.NEGOTIATION_ID_OPTION_GO:
			...
			log.Printf("[Server] Read export name length\n")
			exportName := make([]byte, exportNameLength)
			if _, err := io.ReadFull(conn, exportName); err != nil {
				return err
			}

			log.Printf("[Server] Read export name %s\n", exportName)
			for _, candidate := range exports {
				if candidate.Name == string(exportName) {
					export = candidate

					log.Printf("[Server] Found export %s\n", exportName)
					break
				}
			}

			if export == nil {
				log.Printf("[Server] Export %s not found\n", exportName)
				...
				log.Printf("[Server] Discarded option data\n")
				...
				log.Printf("[Server] Sent error reply\n")
				break
			}
			...
			log.Printf("[Server] Got export size %d\n", size)
			...
			log.Printf("[Server] Discarded information requests\n")
			{
			...
			log.Printf("[Server] Sent export info\n")
			{
				info := &bytes.Buffer{}
			...
			log.Printf("[Server] Sent export name\n")
			{
				info := &bytes.Buffer{}
			...
			log.Printf("[Server] Sent export description\n")
			{
				info := &bytes.Buffer{}
			...
			log.Printf("[Server] Sent block size info\n")
			if err := binary.Write(conn, binary.BigEndian, protocol.NegotiationReplyHeader{
				ReplyMagic: protocol.NEGOTIATION_MAGIC_REPLY,
				ID:         optionHeader.ID,
		...
		case protocol.NEGOTIATION_ID_OPTION_LIST:
			...
			log.Printf("[Server] Sent export list\n")
			if err := binary.Write(conn, binary.BigEndian, protocol.NegotiationReplyHeader{
				ReplyMagic: protocol.NEGOTIATION_MAGIC_REPLY,
				ID:         optionHeader.ID,
				Type:       protocol.NEGOTIATION_TYPE_REPLY_ACK,
		...
		default:
			log.Printf("[Server] Handling unknown option %d\n", optionHeader.ID)
			_, err := io.CopyN(io.Discard, conn, int64(optionHeader.Length)) // Discard the unknown option's data
			if err != nil {
				return err
			}

			log.Printf("[Server] Discarded unknown option data\n")
			if err := binary.Write(conn, binary.BigEndian, protocol.NegotiationReplyHeader{
				ReplyMagic: protocol.NEGOTIATION_MAGIC_REPLY,
				ID:         optionHeader.ID,
