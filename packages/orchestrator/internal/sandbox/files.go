...
func getUffdBinaryPath(fcVersion string) string {
	return filepath.Join(fcVersionsDir, fcVersion, uffdBinaryName)
}

func getFcBinaryPath(fcVersion string) string {
...
func newSandboxFiles(
	...
	childSpan.SetAttributes(
		...
		attribute.String("instance.uffd.path", uffdBinaryPath),
	)
...
func newEnvFiles(
	...
	if err != nil {
		errMsg := fmt.Errorf("failed reading build id for the env %s: %w", envID, err)
		telemetry.ReportCriticalError(childCtx, errMsg)
...
func (env *EnvFiles) Ensure(ctx context.Context) error {
...
func (env *SandboxFiles) Cleanup(
	...
	childCtx, childSpan := tracer.Start(ctx, "cleanup-sandbox-files")
	defer childSpan.End()
...
func (env *EnvFiles) Cleanup(
	ctx context.Context,
	tracer trace.Tracer,
) error {
	childCtx, childSpan := tracer.Start(ctx, "cleanup-env-files",
		trace.WithAttributes(
