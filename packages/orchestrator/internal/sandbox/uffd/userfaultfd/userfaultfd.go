...
type Userfaultfd struct {
	...
	// pageFaultTrace tracks all page faults with timestamp (unix nano) as key and offset as value
	pageFaultTrace   map[int64]int64
...
func NewUserfaultfdFromFd(fd uintptr, src block.Slicer, m *memory.Mapping, logger logger.Logger) (*Userfaultfd, error) {
	...
	u := &Userfaultfd{
		...
		ma:              m,
		pageFaultTrace:  make(map[int64]int64),
		logger:          logger,
	}
...
func (u *Userfaultfd) Serve(
	...
	for {
		shouldContinue, err := u.pollIteration(ctx, pollFds, fdExit, eagainCounter)
		if err != nil {
			return err
		}
...
func (u *Userfaultfd) pollIteration(
	ctx context.Context,
	pollFds []unix.PollFd,
	fdExit *fdexit.FdExit,
	eagainCounter *eagainCounter,
) (shouldContinue bool, err error) {
	ctx, span := tracer.Start(ctx, "poll uffd")
	defer span.End()
	...
	if _, err := unix.Poll(
		...
		return false, fmt.Errorf("failed polling: %w", err)
	}
	...
	if exitFd.Revents&unix.POLLIN != 0 {
		...
		if errMsg != nil {
			...
			return false, fmt.Errorf("failed to handle uffd: %w", errMsg)
		}
	...
	if msgEvent := getMsgEvent(&msg); msgEvent != UFFD_EVENT_PAGEFAULT {
		...
		return false, ErrUnexpectedEventType
	}
	...
	if err != nil {
		...
		return false, fmt.Errorf("failed to map: %w", err)
	}
	...
	if flags&UFFD_PAGEFAULT_FLAG_WRITE != 0 {
		...
		if err != nil {
			return false, fmt.Errorf("failed to handle missing write: %w", err)
		}
	...
	if flags == 0 {
		err := u.handleMissing(ctx, fdExit.SignalExit, addr, pagesize, offset)
		if err != nil {
			return false, fmt.Errorf("failed to handle missing: %w", err)
		}
	...
	return false, fmt.Errorf("unexpected event type: %d, closing uffd", flags)
}
...
func (u *Userfaultfd) handleMissing(
	...
	u.wg.Go(func() error {
		ctx, span := tracer.Start(ctx, "handle missing")
		defer span.End()
		...
		// Track the page fault with timestamp
		u.pageFaultTraceMu.Lock()
		u.pageFaultTrace[time.Now().UnixNano()] = offset
...
// GetPageFaultTrace returns a copy of the page fault trace map (timestamp -> offset).
func (u *Userfaultfd) GetPageFaultTrace() map[int64]int64 {
	u.pageFaultTraceMu.Lock()
	defer u.pageFaultTraceMu.Unlock()

	// Return a copy to avoid data races
	result := make(map[int64]int64, len(u.pageFaultTrace))
	for k, v := range u.pageFaultTrace {
		result[k] = v
	}
	return result
}
