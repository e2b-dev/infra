...
func ConfigureApi(fd uintptr, pagesize uint64) error {
	var features CULong
	...
	ret, _, errno := syscall.Syscall(syscall.SYS_IOCTL, fd, UFFDIO_API, uintptr(unsafe.Pointer(&api)))
	if errno != 0 {
...
func Register(fd uintptr, addr uintptr, size uint64, mode CULong) error {
	register := NewUffdioRegister(CULong(addr), CULong(size), mode)

	ret, _, errno := syscall.Syscall(syscall.SYS_IOCTL, fd, UFFDIO_REGISTER, uintptr(unsafe.Pointer(&register)))
	if errno != 0 {
...
func Copy(fd uintptr, addr uintptr, data []byte, pagesize uint64, mode CULong) error {
	cpy := NewUffdioCopy(data, CULong(addr)&^CULong(pagesize-1), CULong(pagesize), mode, 0)

	if _, _, errno := syscall.Syscall(syscall.SYS_IOCTL, fd, UFFDIO_COPY, uintptr(unsafe.Pointer(&cpy))); errno != 0 {
		return errno
	}
