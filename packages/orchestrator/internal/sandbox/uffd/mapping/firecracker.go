...
func (m FcMappings) GetRange(addr uintptr) (uint64, uint64, uint64, error) {
	for _, m := range m {
		...
		return m.relativeOffset(addr), uint64(m.PageSize), uint64(m.Size), nil
	}

	return 0, 0, 0, fmt.Errorf("address %d not found in any mapping", addr)
}
