...
func GetHostMetrics() (*HostMetrics, error) {
	...
	// If no disks were found, at least report root filesystem
	if len(disks) == 0 {
		...
		if err == nil {
			diskInfo := DiskInfo{
				...
				UsedMB:         int64(rootUsage.Used / 1024 / 1024),
				TotalMB:        int64(rootUsage.Total / 1024 / 1024),
				UsedPercent:    rootUsage.UsedPercent,
			}
			disks = append(disks, diskInfo)
		}
	}
