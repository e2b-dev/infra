...
func TestServer_Update_MetadataOnly(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			wantErr: false,
			validateFunc: func(t *testing.T, sbx *sandbox.Sandbox) {
				assert.NotNil(t, sbx.Config)
				assert.NotNil(t, sbx.Config.Metadata)

				// Should replace existing
				assert.Equal(t, "", sbx.Config.Metadata["existing"])
				assert.Equal(t, "value1", sbx.Config.Metadata["key1"])
				assert.Equal(t, "value2", sbx.Config.Metadata["key2"])
				assert.Len(t, sbx.Config.Metadata, 2)
			},
		},
		{
			...
			validateFunc: func(t *testing.T, sbx *sandbox.Sandbox) {
				...
				assert.Equal(t, "new-value", sbx.Config.Metadata["existing"])
				assert.Equal(t, "new-data", sbx.Config.Metadata["new-key"])
				assert.Len(t, sbx.Config.Metadata, 2)
			},
		},
		{
			...
			validateFunc: func(t *testing.T, sbx *sandbox.Sandbox) {
				assert.NotNil(t, sbx.Config.Metadata)
				assert.Equal(t, "metadata", sbx.Config.Metadata["first"])
				assert.Len(t, sbx.Config.Metadata, 1)
			},
		},
		{
			...
			validateFunc: func(t *testing.T, sbx *sandbox.Sandbox) {
				...
				assert.NotNil(t, sbx.Config.Metadata)
				assert.Len(t, sbx.Config.Metadata, 0)
			},
		},
		{
...
func TestServer_Update_EndTimeAndMetadata(t *testing.T) {
	...
	assert.NotNil(t, updatedSandbox.Config.Metadata)
	assert.Equal(t, "updated-value", updatedSandbox.Config.Metadata["original"])
	assert.Equal(t, "metadata", updatedSandbox.Config.Metadata["new"])
	assert.Len(t, updatedSandbox.Config.Metadata, 2)
}
...
func TestServer_Update_OnlyEndTime(t *testing.T) {
	...
	assert.NotNil(t, updatedSandbox.Config.Metadata)
	assert.Equal(t, "remain-unchanged", updatedSandbox.Config.Metadata["should"])
	assert.Len(t, updatedSandbox.Config.Metadata, 1)
}
