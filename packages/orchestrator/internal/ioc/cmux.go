...
func startCMUXServer(
	...
	invokeAsync(s, func() error {
		if err := healthHTTPServer.Serve(httpListener); ignoreUseOfClosed(err) != nil {
			return err
		}
...
const (
	preShutdownTimeout  = 24 * time.Hour
...
func stopCMUXServer(
	...
	if err := healthHTTPServer.Shutdown(ctx); ignoreUseOfClosed(err) != nil {
	...
	logger.Info(ctx, "cmux server is closed")
}
