...
func (ppb *PostProcessingBuilder) Build(
	...
	if startMetadata == nil && ppb.Config.FromTemplate != nil {
		tm, err := ReadTemplateMetadata(ctx, ppb.templateStorage, ppb.Config.FromTemplate.BuildID)
		if err != nil {
			return LayerResult{}, fmt.Errorf("error reading from template metadata: %w", err)
		}
	...
	if err != nil {
		return LayerResult{}, fmt.Errorf("error running start and ready commands in sandbox: %w", err)
	}
