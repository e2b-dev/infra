...
func (c *Copy) Execute(
	...
	if err != nil {
		return sandboxtools.CommandMetadata{}, fmt.Errorf("open files object: %w", err)
	}
	...
	if err != nil {
		return sandboxtools.CommandMetadata{}, fmt.Errorf("create temp file: %w", err)
	}
	...
	if err != nil {
		return sandboxtools.CommandMetadata{}, fmt.Errorf("copy data to temp file: %w", err)
	}
	...
	if err != nil {
		return sandboxtools.CommandMetadata{}, fmt.Errorf("copy data to sandbox: %w", err)
	}
	...
	if err != nil {
		return sandboxtools.CommandMetadata{}, fmt.Errorf("extract files: %w", err)
	}
	...
	if err != nil {
		return sandboxtools.CommandMetadata{}, fmt.Errorf("execute copy script: %w", err)
	}
	...
	if err != nil {
		return sandboxtools.CommandMetadata{}, fmt.Errorf("move files: %w", err)
	}
	...
	if len(args) >= 3 {
		...
		if owner != "" {
			...
			if err != nil {
				return sandboxtools.CommandMetadata{}, fmt.Errorf("set ownership and permissions: %w", err)
			}
		}
	}
