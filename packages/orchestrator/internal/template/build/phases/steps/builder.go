...
func (sb *StepsBuilder) Build(
	...
	for i, step := range sb.Config.Steps {
		...
		if err != nil {
			return phases.LayerResult{}, fmt.Errorf("check if step %d should be built: %w", i+1, err)
		}
		...
		if err != nil {
			return phases.LayerResult{}, fmt.Errorf("build step %d: %w", i+1, err)
		}
...
func (sb *StepsBuilder) shouldBuildStep(
	...
	if !forceBuild {
		...
		if err != nil {
			...
			if err != nil {
				return phases.LayerResult{}, fmt.Errorf("check if layer cached: %w", err)
			}
...
func (sb *StepsBuilder) buildStep(
	...
	actionExecutor := layer.NewFunctionAction(func(ctx context.Context, sbx *sandbox.Sandbox, cmdMeta sandboxtools.CommandMetadata) (sandboxtools.CommandMetadata, error) {
		...
		if err != nil {
			return sandboxtools.CommandMetadata{}, fmt.Errorf("process layer: %w", err)
		}
		...
		if err != nil {
			return sandboxtools.CommandMetadata{}, fmt.Errorf("run sync command: %w", err)
		}
	...
	if err != nil {
		return phases.LayerResult{}, fmt.Errorf("run build layer: %w", err)
	}
