...
func (b *Builder) Build(ctx context.Context, finalMetadata storage.TemplateFiles, template config.TemplateConfig, logsWriter *zap.Logger) (r *Result, e error) {
	...
	defer func(ctx context.Context) {
		...
		if removeErr != nil {
			e = errors.Join(e, fmt.Errorf("remove build files: %w", removeErr))
		}
	}(context.WithoutCancel(ctx))

	envdVersion, err := envd.GetEnvdVersion(ctx)
	if err != nil {
		return nil, fmt.Errorf("get envd version: %w", err)
	}
	...
	defer func() {
		...
		if err != nil {
			e = errors.Join(e, fmt.Errorf("upload template layers: %w", err))
		}
	}()
...
func runBuild(
	...
	if err != nil {
		return nil, fmt.Errorf("wait for layers upload: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("get rootfs size: %w", err)
	}
...
func getRootfsSize(
	...
	if err != nil {
		return 0, fmt.Errorf("open rootfs header: %w", err)
	}

	h, err := header.Deserialize(obj)
	if err != nil {
		return 0, fmt.Errorf("deserialize rootfs header: %w", err)
	}
