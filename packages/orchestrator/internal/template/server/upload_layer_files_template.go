...
func (s *ServerStore) TemplateLayerFilesUpload(ctx context.Context, in *templatemanager.TemplateLayerFilesUploadRequest) (*templatemanager.TemplateLayerFilesUploadResponse, error) {
	...
	obj, err := s.storage.OpenObject(ctx, builder.GetLayerFilesCachePath(in.TemplateID, in.Hash))
	if err != nil {
	...
	if err != nil {
		...
		signedUrl, err := s.storage.SignedURL(ctx, builder.GetLayerFilesCachePath(in.TemplateID, in.Hash), signedUrlExpiration)
		if err != nil {
