...
func BenchmarkBaseImageLaunch(b *testing.B) {
	...
	persistence, err := storage.GetStorageProvider(b.Context(), storage.TemplateStorageConfig, limiter)
	require.NoError(b, err)
	...
	persistenceTemplate, err := storage.GetStorageProvider(b.Context(), storage.TemplateStorageConfig, nil)
	require.NoError(b, err)

	persistenceBuild, err := storage.GetStorageProvider(b.Context(), storage.BuildCacheStorageConfig, nil)
	require.NoError(b, err)
