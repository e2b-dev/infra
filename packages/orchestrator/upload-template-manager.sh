...
case "$PROVIDER" in
  ...
  linux)
    ...
    if [ -n "$ARTIFACT_SCP_SSH_KEY" ]; then
      scp -P "$ARTIFACT_SCP_PORT" -i "$ARTIFACT_SCP_SSH_KEY" bin/orchestrator "${ARTIFACT_SCP_USER:+$ARTIFACT_SCP_USER@}$ARTIFACT_SCP_HOST:$ARTIFACT_SCP_DIR/template-manager"
    else
      scp -P "$ARTIFACT_SCP_PORT" bin/orchestrator "${ARTIFACT_SCP_USER:+$ARTIFACT_SCP_USER@}$ARTIFACT_SCP_HOST:$ARTIFACT_SCP_DIR/template-manager"
    fi
    ;;
  *)
