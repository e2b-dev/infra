...
case "$PROVIDER" in
  ...
  linux)
    ...
    if [ -n "$ARTIFACT_HTTP_SSH_KEY" ]; then
      scp -P "$ARTIFACT_HTTP_PORT" -i "$ARTIFACT_HTTP_SSH_KEY" bin/orchestrator "${ARTIFACT_HTTP_USER:+$ARTIFACT_HTTP_USER@}$ARTIFACT_HTTP_HOST:$ARTIFACT_HTTP_DIR/template-manager"
    else
      scp -P "$ARTIFACT_HTTP_PORT" bin/orchestrator "${ARTIFACT_HTTP_USER:+$ARTIFACT_HTTP_USER@}$ARTIFACT_HTTP_HOST:$ARTIFACT_HTTP_DIR/template-manager"
    fi
    ;;
  *)
