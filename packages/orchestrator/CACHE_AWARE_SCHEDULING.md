...
The orchestrator already returns `SchedulingMetadata` for creates, pauses, checkpoints, and template builds. The metadata describes build lineage overlap for memfile and rootfs artifacts: base build ID, current build ID, referenced build IDs, and referenced bytes.
