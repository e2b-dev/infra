...
type RedisServiceDiscoveryConfig struct {
	RedisClient *redis.Client
	Logger      *zap.Logger
...
func NewRedisServiceDiscovery(ctx context.Context, conf *RedisServiceDiscoveryConfig) ServiceDiscoveryAdapter {
	...
	go func() { sd.selfRegistration(ctx) }()
...
func (sd *ServiceDiscovery) GetSelfNodeId() string {
	return sd.serviceId
}

func (sd *ServiceDiscovery) SetSelfStatus(status string) {
...
func (sd *ServiceDiscovery) selfRegistration(ctx context.Context) {
	...
	for {
		...
		case <-ctx.Done():
			...
			return
		case <-ticker.C:
			err := sd.executeSelfRegistration(ctx)
			if err != nil {
...
func (sd *ServiceDiscovery) executeSelfRegistration(ctx context.Context) error {
	err := sd.redisMx.Lock()
	if err != nil {
		return lockError
	}
