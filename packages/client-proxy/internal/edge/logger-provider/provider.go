...
type LogsQueryProvider interface {
	...
	QuerySandboxLogsV2(ctx context.Context, teamID string, sandboxID string, start time.Time, end time.Time, limit int, direction logproto.Direction) ([]logs.LogEntry, error)
}
