...
-- +goose StatementBegin
RENAME TABLE sandbox_logs_local TO sandbox_logs_local_legacy;
-- +goose StatementEnd
...
CREATE TABLE sandbox_logs AS sandbox_logs_local
    ENGINE = Distributed('cluster', currentDatabase(), 'sandbox_logs_local', xxHash64(LogAttributes['team_id']));
-- +goose StatementEnd

-- +goose StatementBegin
INSERT INTO sandbox_logs_local (
    Timestamp,
    TraceId,
    SpanId,
    TraceFlags,
...
SELECT
    ...
    0,
    lower(level),
    multiIf(lower(level) = 'debug', 5, lower(level) = 'warn', 13, lower(level) = 'error', 17, 9),
    service,
    message,
    '',
    map(),
    ...
    ''
FROM sandbox_logs_local_legacy;
-- +goose StatementEnd
...
-- +goose StatementBegin
DROP TABLE sandbox_logs_local;
-- +goose StatementEnd

-- +goose StatementBegin
RENAME TABLE sandbox_logs_local_legacy TO sandbox_logs_local;
-- +goose StatementEnd
