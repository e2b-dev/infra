...
func mustFeatureFlagCounter(name, description string) metric.Int64Counter {
	counter, err := otel.Meter("github.com/e2b-dev/infra/packages/shared/pkg/featureflags").Int64Counter(name, metric.WithDescription(description))
	if err != nil {
		return nil
	}
...
// Named under orchestrator.* so it passes the
// otel collector's include-only metric allow-list.
var firecrackerVersionResolutionMetric = mustFeatureFlagCounter(
