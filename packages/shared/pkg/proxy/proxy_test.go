...
func TestProxySnapshotInProgressErrorTemplate(t *testing.T) {
	t.Parallel()

	getDestination := func(*http.Request) (*pool.Destination, error) {
		return nil, NewErrSandboxSnapshotInProgress("test-sandbox", "sandbox snapshot is currently being created")
	}
	...
	t.Run("json for non-browser", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.Equal(t, "test-sandbox", response.SandboxID)
		require.Equal(t, "sandbox snapshot is currently being created", response.Message)
	...
	t.Run("html for browser", func(t *testing.T) {
		...
		body, err := io.ReadAll(resp.Body)
		require.NoError(t, err)
		assert.Contains(t, string(body), "Sandbox Snapshot In Progress")
		assert.Contains(t, string(body), "test-sandbox")
	})
}
