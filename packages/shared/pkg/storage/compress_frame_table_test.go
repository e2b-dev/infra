...
func TestRange(t *testing.T) {
	...
	t.Run("selects single middle frame", func(t *testing.T) {
		...
		require.Equal(t, int64(1<<20), offsets[0].U)
		require.Equal(t, int64(500_000), offsets[0].C)
	})
	...
	t.Run("respects StartAt on subset", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.Len(t, offsets, 1)
		require.Equal(t, int64(2<<20), offsets[0].U)
		require.Equal(t, int64(1_100_000), offsets[0].C) // 500k + 600k

		// Query for offset 0 — before the subset, should find nothing.
		offsets, err = collectRange(sub, 0, 1<<20)
		require.NoError(t, err)
		require.Empty(t, offsets, "Range should not find frames before StartAt")
	})
}
...
func TestSubset(t *testing.T) {
	...
	t.Run("full range", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.Len(t, sub.Frames, 3)
		require.Equal(t, int64(0), sub.StartAt.U)
	})
	...
	t.Run("last frame", func(t *testing.T) {
		...
		require.Equal(t, int64(2<<20), sub.StartAt.U)
		require.Equal(t, int64(1_100_000), sub.StartAt.C)
		require.Equal(t, int32(400_000), sub.Frames[0].C)
	})
	...
	t.Run("preserves compression type", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.Equal(t, CompressionLZ4, sub.CompressionType())
	})
...
func TestGetFetchRange(t *testing.T) {
	...
	t.Run("translates U-space to C-space", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.Equal(t, int64(500_000), r.Start)
		require.Equal(t, 600_000, r.Length)
	})
