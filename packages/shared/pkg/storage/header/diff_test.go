...
func TestCreateDiff_Hugepage(t *testing.T) {
	...
	m, err := WriteDiff(t.Context(), source, int64(blockSize), dirty, diff)
...
func TestCreateDiff_UnsupportedBlockSize(t *testing.T) {
	...
	diff := bytes.NewBuffer(nil)
	_, err := WriteDiff(t.Context(), source, int64(blockSize), dirty, diff)
...
func TestCreateDiff_LargeIndex(t *testing.T) {
	...
	diff := bytes.NewBuffer(nil)
	m, err := WriteDiff(t.Context(), largeSource, int64(blockSize), dirty, diff)
	assert.NoError(t, err)
