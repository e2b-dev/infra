...
func TestCreateDiff_Hugepage(t *testing.T) {
	...
	_, emptyBlocks, err := WriteDiff(source, int64(blockSize), dirty, diff)
...
func TestCreateDiff_UnsupportedBlockSize(t *testing.T) {
	...
	diff := bytes.NewBuffer(nil)
	_, _, err := WriteDiff(source, int64(blockSize), dirty, diff)
...
func TestCreateDiff_EmptyDirtyBitset(t *testing.T) {
	...
	diff := bytes.NewBuffer(nil)
	dirtyResult, emptyBlocks, err := WriteDiff(source, int64(blockSize), dirty, diff)
	assert.NoError(t, err)
...
func TestCreateDiff_LargeIndex(t *testing.T) {
	...
	diff := bytes.NewBuffer(nil)
	dirtyResult, emptyBlocks, err := WriteDiff(largeSource, int64(blockSize), dirty, diff)
	assert.NoError(t, err)
