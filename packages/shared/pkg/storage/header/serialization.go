...
// StoreHeader serializes a header and uploads it to long-term storage, also
// returning the serialized bytes so the caller can reuse them in-process
// (e.g. for inline delivery to peer clients) without re-serializing. Refuses
// to persist a header still flagged as in-flight — the upload pipeline must
// clear IncompletePendingUpload before reaching here.
func StoreHeader(ctx context.Context, s storage.StorageProvider, path string, h *Header) ([]byte, error) {
	if h.IncompletePendingUpload {
		return nil, fmt.Errorf("refusing to persist incomplete header for %s", path)
	}
