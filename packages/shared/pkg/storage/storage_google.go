...
// openRangeReader opens a streaming reader for the given byte range. Each
// attempt (open + reads + close) runs under its own context.WithTimeout; on
// transient failure the reader is reopened at baseOff+bytesRead, up to the
// shared attempt budget. Uses seekableReadHandler so the wrapper is the only
// retry layer.
