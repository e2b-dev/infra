...
func (c *cachedSeekable) openReaderCompressed(ctx context.Context, offsetU int64, frameTable *FrameTable) (RangeReader, error) {
	...
	if !skipCacheWriteback(ctx) {
		in = newCaptureReader(raw, r.Length, true,
			c.compressedFrameWriteback(path, offsetU, r.Length, trace.SpanContextFromContext(ctx)))
	}
...
func (c *cachedSeekable) compressedFrameWriteback(framePath string, offset int64, expectedSize int, fetchSpan trace.SpanContext) func(context.Context, []byte) {
	return func(ctx context.Context, frame []byte) {
		...
		c.goCtx(ctx, func(ctx context.Context) {
			ctx, span := c.tracer.Start(ctx, "write compressed frame back to cache", writebackSpanOpts(fetchSpan)...)
			defer span.End()
