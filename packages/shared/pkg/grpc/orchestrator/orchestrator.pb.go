...
type SandboxCheckpointRequest struct {
	...
	// When true, persist only the filesystem (no memory snapshot); the sandbox
	// still resumes in place, but the produced template is rootfs-only and
	// cold-boots (reboots) on launch. Default false, so existing callers are
	// unaffected.
