...
func (db *DB) UpdateEnvLastUsed(ctx context.Context, count int64, envID string) (err error) {
	return db.Client.Env.UpdateOneID(envID).AddSpawnCount(count).SetLastSpawnedAt(time.Now()).Exec(ctx)
}
