...
func (g *LocalArtifactsRegistry) GetLayer(ctx context.Context, buildId string, layerHash string, platform containerregistry.Platform) (containerregistry.Image, error) {
	panic("GetLayer is not implemented for LocalArtifactsRegistry")
}

func (g *LocalArtifactsRegistry) PushLayer(ctx context.Context, buildId string, layerHash string, layer containerregistry.Image) error {
