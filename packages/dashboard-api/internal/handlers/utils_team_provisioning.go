...
func (s *APIStore) createHeadlessTeam(ctx context.Context, name string, email string) (provisionedTeam, error) {
	...
	if err != nil {
		return provisionedTeam{}, fmt.Errorf("create headless team: %w", err)
	}
	...
	if err := s.teamProvisionSink.ProvisionTeam(ctx, req); err != nil {
		...
		if deleteErr := s.authDB.Write.DeleteTeamByID(rollbackCtx, team.ID); deleteErr != nil {
			return provisionedTeam{}, fmt.Errorf("delete headless team after provisioning failure: provision=%s delete=%w", err.Error(), deleteErr)
		}
