...
func TestPatchAgentsAgentID_UpdatesTeamAgentAndClearsNullableFields(t *testing.T) {
	...
	agentID := uuid.New()
	require.NoError(t, testDB.SqlcClient.TestsRawSQL(ctx, `
		INSERT INTO public.agents (id, team_id, name, template_id, description, command, author, metadata, public)
		VALUES ($1, $2, 'Codex', 'codex-template', 'Codex CLI.', 'codex', 'E2B', '{"icon":"terminal"}', false)
	`, agentID, teamID))
