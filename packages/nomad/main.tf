...
data "docker_registry_image" "api_migrator_image" {
  name = "${var.gcp_region}-docker.pkg.dev/${var.gcp_project_id}/${var.orchestration_repository_name}/api-migrator:latest"
}

resource "docker_image" "api_migrator_image" {
  name          = data.docker_registry_image.api_migrator_image.name
  pull_triggers = [data.docker_registry_image.api_migrator_image.sha256_digest]
  platform      = "linux/amd64/v8"
}
...
resource "nomad_job" "api" {
  jobspec = templatefile("${path.module}/api.hcl", {
    ...
    api_migrator_docker_image = docker_image.api_migrator_image.repo_digest
  })
}
