...
function generate_nomad_config {
  ...
  local readonly orchestrator_commit_hash="$5"
...
client {
  ...
  meta {
    node_pool = "default"
    orchestrator_commit_hash= "$orchestrator_commit_hash"
  }
}
...
function run {
  ...
  local orchestrator_commit_hash=""
  local all_args=()
  ...
  while [[ $# > 0 ]]; do
    ...
    --num-servers)
      ...
      shift
      ;;
    --orchestrator-commit-hash)
      assert_not_empty "$key" "$2"
      orchestrator_commit_hash="$2"
      shift
      ;;
    --consul-token)
      assert_not_empty "$key" "$2"
  ...
  if [[ "$skip_nomad_config" == "true" ]]; then
    ...
    generate_nomad_config "$num_servers" "$config_dir" "$user" "$consul_token" "$orchestrator_commit_hash"
  fi

  generate_supervisor_config "$SUPERVISOR_CONFIG_PATH" "$config_dir" "$data_dir" "$bin_dir" "$log_dir" "$user"
