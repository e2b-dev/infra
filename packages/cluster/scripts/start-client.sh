...
gsutil cp "gs://${SCRIPTS_BUCKET}/run-consul-client-${RUN_CONSUL_FILE_HASH}.sh" /opt/consul/bin/run-consul.sh
gsutil cp "gs://${SCRIPTS_BUCKET}/run-nomad-client-${RUN_NOMAD_FILE_HASH}.sh" /opt/nomad/bin/run-nomad.sh
