...
gsutil cp "gs://${SCRIPTS_BUCKET}/run-client-nomad-${RUN_NOMAD_FILE_HASH}.sh" /opt/nomad/bin/run-nomad.sh
...
/opt/nomad/bin/run-nomad.sh --consul-token "${CONSUL_TOKEN}" "${ORCHESTRATOR_COMMIT_HASH}" &
