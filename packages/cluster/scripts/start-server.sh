...
gsutil cp "gs://${SCRIPTS_BUCKET}/run-consul-server-${RUN_CONSUL_FILE_HASH}.sh" /opt/consul/bin/run-consul.sh
gsutil cp "gs://${SCRIPTS_BUCKET}/run-nomad-server-${RUN_NOMAD_FILE_HASH}.sh" /opt/nomad/bin/run-nomad.sh
...
/opt/nomad/bin/run-nomad.sh --num-servers "${NUM_SERVERS}" --consul-token "${CONSUL_TOKEN}" --nomad-token "${NOMAD_TOKEN}"
