...
func (g *GCS) ReadAt(b []byte, off int64) error {
	...
	_, err = reader.Read(g.buffer[off : off+int64(len(b))])
	if err != nil {
		return err
	}

	g.bufferLock.Lock()
	copy(b, g.buffer[off:off+int64(len(b))])
	g.bufferLock.Unlock()
...
func (g *GCS) WriteAt(p []byte, off int64) error {
