...
> Note: Linux is required for developing on bare metal. This is a work in progress. Not everything will function as
> expected.

1. `sudo modprobe nbd nbds_max=64`
2. `sudo sysctl -w vm.nr_hugepages=512` enable huge pages
...
9. `cd packages/orchestrator/ && make build-debug && sudo make run-local` run the orchestrator and template-manager
   locally.
10. `cd packages/client-proxy && make run-local` run the client-proxy locally.
11. `cd packages/api && make run-local` run the api locally
