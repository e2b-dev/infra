...
while IFS= read -r -d '' file; do
    ...
    if grep -q "$search_pattern" "$file"; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # Show the change
            echo "  Changed to: var tracer = otel.Tracer(\"$relative_path\")"
        fi
    else
        ((skipped++))
    fi
...
# Option to create a more specific version for certain file types
if [ "$2" == "--dry-run" ]; then
    echo ""
    echo "This was a dry run. To actually make changes, run without --dry-run"
fi
