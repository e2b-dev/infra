...
backend: the control-plane REST API, the default data-plane VM orchestration built on
**Firecracker microVMs**, the in-VM agent, the edge routing layer, template building, and the
Terraform/Nomad infrastructure to deploy it all on GCP (AWS in beta). The Indentia provider adds
an explicit phase-one alternative: OCI sandboxes in Kubernetes Pods isolated by the `kata-clh`
or `kata-qemu` RuntimeClass, without Firecracker or Nomad.
...
flowchart TB
    ...
    subgraph k8s["Alternative Kubernetes/OKD data plane"]
        KORCH["kubernetes-orchestrator<br/>gRPC :5008, proxy :5007"]
        KPOD["sandbox Pod<br/>kata-clh or kata-qemu<br/>envd :49983"]
        KORCH --> KPOD
    end
...
| Orchestrator | `packages/orchestrator` | sandbox node or Kubernetes controller | Runs Firecracker VMs, or Kata Pods for the phase-one OKD backend |
...
The default Go binary runs on every sandbox node as root. `ORCHESTRATOR_SERVICES` selects its
roles: `orchestrator` (run sandboxes) and/or `template-manager` (build templates). Code lives
under `pkg/`, almost all Linux-only. A separate
`cmd/kubernetes-orchestrator` binary implements the same sandbox/info/proxy boundary with a
namespace-scoped Kubernetes client and does not need host privileges.
...
`pkg/kubernetesserver/` is a deep alternative SandboxService implementation. It resolves an OCI
image from `{template_id}` and `{build_id}`, chooses an operator-allowed RuntimeClass from the
reserved `e2b.runtime-class` metadata key, and owns a Pod, Secret, and NetworkPolicy per sandbox.
`kata-clh` is the default and `kata-qemu` is the supported fallback. Create waits for Pod Ready
and a successful envd `/init`; List reconstructs the orphan-detection fields from Pod metadata;
the proxy resolves the current Pod IP and credentials from namespace-scoped informer caches for
every request, without putting Kubernetes API calls in the traffic hot path. Snapshot resume, Pause, Checkpoint,
auto-pause/resume, volumes, IAM, and BYOP/domain egress fail before Pod creation. Firecracker-only
configuration fields such as huge-page and version hints are ignored. Every sandbox NetworkPolicy
denies E2B's shared private, loopback, link-local, and metadata CIDRs by default; user CIDR allow
lists cannot reopen those protected ranges.
ChunkService and VolumeService therefore remain unimplemented in this phase.
The InfoService advertises the reserved `e2b-runtime-oci-kata` label; placement then checks only
CPU architecture for this backend because an OCI cold boot does not restore CPU-specific VM
memory. An explicit `e2b.runtime-class` request also becomes a mandatory placement label together
with `e2b-runtime-oci-kata`, preventing it from landing on a Firecracker node in a mixed fleet.
Firecracker nodes retain the stricter family/model snapshot compatibility check.
...
For the Kubernetes backend the API flow and gRPC contract are unchanged, but the implementation
is OCI/Pod based: the controller creates a Secret and ingress/egress NetworkPolicy, starts one Pod
with `runtimeClassName: kata-clh` or `kata-qemu`, waits for its envd health probe, posts `/init`,
then returns. Template builds must already be published as OCI images containing `/usr/bin/envd`;
the Firecracker object-artifact format is not translated at runtime. Its egress NetworkPolicy
always excludes private, loopback, link-local, and metadata ranges even when the caller supplies no
egress policy; supported CIDR policies can narrow access but cannot reopen that baseline.
...
sequenceDiagram
    ...
    OP->>E: http://slotIP-or-PodIP:3000
    E-->>U: response
```
...
This section describes the Firecracker backend. The Kubernetes/Kata phase-one backend returns
`Unimplemented` for Pause and Checkpoint and rejects auto-pause/resume at Create; Pod recreation
is not presented as snapshot or memory-resume semantics.
...
The default providers deploy with **Terraform** (`iac/provider-gcp/`, `iac/provider-aws/`) onto a
**Nomad + Consul** cluster. Nomad job specs live in `iac/modules/job-*/jobs/*.hcl`.
...
`iac/provider-okd/` is the alternative Kubernetes topology. It deploys one stable-identity,
non-host-networked StatefulSet plus namespace-scoped RBAC. The API discovers that Pod directly;
when a restart changes its Pod IP, discovery replaces the stale gRPC/proxy node connection while
preserving the StatefulSet Pod name as the E2B node ID. The controller creates sandbox Pods on
Kata-labelled nodes. Both controller and sandbox Pods are
non-privileged. A dedicated tokenless sandbox ServiceAccount receives OpenShift's non-privileged
`anyuid` SCC because envd runs as UID 0 inside the Kata guest; host access and privilege escalation
remain disabled. Per-sandbox Secrets hold only proxy traffic-token and host-mask material; envd
initialization stays in the controller request path rather than being persisted in Kubernetes. The
NetworkPolicies admit sandbox ingress only from the controller. The provider has a read-only
preflight and an explicit `OKD_APPLY=YES` mutation guard; it does not deploy Nomad, Firecracker,
custom or privileged SCCs, host devices, or host mounts.
...
iac/                    Terraform/Nomad providers plus provider-okd Kubernetes/Kata manifests
