...
#
# IMPORTANT: This data source fails on Nomad API errors (network, auth, TLS) to prevent
...
data "external" "template_manager_count" {
  program = ["bash", "-c", <<-EOF
    ...
    # Use ---HTTP_STATUS: as marker to avoid Terraform template escaping issues with %
    RESPONSE=$(curl -s -w "\n---HTTP_STATUS:%%{http_code}" -H "X-Nomad-Token: $TOKEN" \
      "$ADDR/v1/job/template-manager" 2>&1)
    CURL_EXIT=$?
    ...
    if [ "$HTTP_CODE" = "404" ]; then
      ...
      echo "ERROR: Nomad API returned HTTP $HTTP_CODE for job/template-manager" >&2
