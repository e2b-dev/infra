...
function print_usage {
  ...
  echo -e "  --orchestrator-version\tThe orchestrator version to set as node metadata. Optional."
  echo
  echo "Example:"
  echo
...
function generate_nomad_config {
  ...
  local -r node_pool="$7"
  local -r orchestrator_version="$8"
...
client {
  ...
  meta {
    ...
    ${orchestrator_version:+"\"orchestrator_version\"" = "\"$orchestrator_version\""}
  }
...
function run {
  ...
  generate_nomad_config "$server" "$client" "$num_servers" "$config_dir" "$user" "$consul_token" "$node_pool" "$orchestrator_version"
