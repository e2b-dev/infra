locals {
  client_pool_name_c4 = "${var.prefix}${var.client_cluster_name}-c4"
  client_startup_script_c4 = templatefile("${path.module}/scripts/start-client-c4.sh", {
...
resource "google_compute_health_check" "client_nomad_check_c4" {
  name                = "${local.client_pool_name_c4}-nomad-client-check-c4"
...
resource "google_compute_region_autoscaler" "client_c4" {
  count = var.client_cluster_size < var.client_cluster_size_max ? 1 : 0

  name   = "${local.client_pool_name_c4}-client-autoscale-c4r"
  region = var.gcp_region
  target = google_compute_region_instance_group_manager.client_pool_c4.id
...
resource "google_compute_region_instance_group_manager" "client_pool_c4" {
  name   = "${local.client_pool_name_c4}-ig"
  region = var.gcp_region

  target_size = var.client_cluster_size < var.client_cluster_size_max ? null : var.client_cluster_size

  version {
    name              = google_compute_instance_template.client_c4.id
    instance_template = google_compute_instance_template.client_c4.id
  }

  auto_healing_policies {
    health_check      = google_compute_health_check.client_nomad_check_c4.id
  ...
  base_instance_name = local.client_pool_name_c4
  target_pools       = []

  depends_on = [
    google_compute_instance_template.client_c4,
  ]
}


resource "google_compute_instance_template" "client_c4" {
  name_prefix = "${local.client_pool_name_c4}-c4-"
  ...
  metadata_startup_script = local.client_startup_script_c4
  metadata = {
