#!/bin/bash
# NX.2.5 Hetzner-adapted from e2b-dev/infra/iac/provider-aws/nomad-cluster/scripts/.
# AWS metadata URLs → Hetzner metadata URLs (169.254.169.254/hetzner/v1/metadata).
# AWS CLI s3 calls → mc (MinIO Client) for Hetzner Object Storage.
# IAM-based S3 access → S3 access keys passed via cloud-init.
...
readonly EC2_INSTANCE_METADATA_URL="http://169.254.169.254/hetzner/v1/metadata"
readonly EC2_DYNAMIC_METADATA_URL="http://169.254.169.254/hetzner/v1/metadata"
readonly HETZNER_METADATA_URL="http://169.254.169.254/hetzner/v1/metadata"
